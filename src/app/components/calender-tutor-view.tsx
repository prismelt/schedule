...
import CalendarDayTutorView from "./calender-day-tutor-view";
import PopupTutorView from "./popup-tutor-view";

function Calendar() {
  const [currentDate, setCurrentDate] = useState(new Date());
  const [selectedDate, setSelectedDate] = useState<Date | null>(null);
  const { data: requests } = api.tutorView.getAllTutorFutureView.useQuery();
  ...
  const getAllRequestsFor = (date: Date) => {
    if (!requests) return [];
    ...
    <div className={styles.calendar}>
      ...
      <div className={styles.daysGrid}>
        {getDaysInMonth().map((date, index) => (
          <CalendarDayTutorView
            key={index}
            date={date}
            isPassed={date < today}
            isActive={![0, 1, 6].includes(date.getDay())}
            isEmpty={isEmpty(date)}
            requestsLength={getAllRequestsFor(date).length}
      ...
      {selectedDate && (
        <PopupTutorView
          date={selectedDate}
          request={getAllRequestsFor(selectedDate)}
          onClose={closeModal}
        />
      )}
    </div>
